...
const VesselMap = ({ newCoordinates }) => {
  const locations = [
    {
  ...
  useEffect(() => {
    ...
    locations.forEach(loc => {
      ...
      const popupContent = 
        <div className="p-2">
          <h3 className="font-bold">{loc.ObjectType}</h3>
    ...
    if (newCoordinates) {
      ...
      L.marker(newCoordinates, { icon: createCustomIcon(fishingVesselSvg, '#28a745') }) // Example color for new coordinates
        .addTo(map)
        .bindPopup("New Coordinates")
        .openPopup();
...
export default function Dashboard() {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      // Assuming the backend returns an object with latitude and longitude
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="px-4 py-6 sm:px-0">
          <h2 className="text-2xl font-bold mb-4">Welcome to the maps</h2>
          <p className="text-gray-700">
            Here you can view curated images and maps with the latest detailed reports.
          </p>
          <VesselMap newCoordinates={newCoordinates} /> {/* Pass new coordinates to VesselMap */}
